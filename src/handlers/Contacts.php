...
class Contacts extends Handler
{
    public function handle(): string
    {
        ...
        elseif (!empty($_GET['edit'])) {
            $formData = Database::instance()->getContactById($user->getId(), (int)$_GET['edit']);
        }
        elseif (!empty($_GET['delete'])) {
            Database::instance()->deleteContactById($user->getId(), (int)$_GET['delete']);
            $this->requestRedirect('/contacts');
            return '';
        }
        return (new Template('contacts'))->render([
            'user' => $user,
            'contacts' => Database::instance()->getContacts($user->getId()),
            'formError' => $formError,
            'formData' => $formData,
        ]);
    }
